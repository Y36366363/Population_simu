...
这是一个可运行的机制实验框架。第二版加入“个人—家庭分支—姓氏家族—国家—政策时间线”，用于探索资源集中、生育数量、阶层跃迁与家族延续之间的长期联动。它用于提出问题和比较情景，不用于直接预测真实国家人口。

交互式网页沙盘（合并到 `main` 并启用 GitHub Pages 后）：
...
`docs/` 包含不依赖服务器的浏览器实验器，可调节初始资源、模拟代数、托育、祖辈照护、动态追投、弱势补偿、公共教育、福利和住房压力，并即时比较一孩、二孩、三孩方案。
