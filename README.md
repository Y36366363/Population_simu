...
本次网页改动已在本地应用中验证：默认世界视图、Python 年度时间线、国家/地区/政策对比、CSV 下载、迁徙开放度开关和家庭资源子命题均可用，浏览器控制台无错误；Python 回归测试为 45 项全部通过。
...
7. 将综合 `amenity_supply` 拆成学校、托育、医疗、交通和安全五个可观测维度，使用地区面板数据做滞后效应和异质性估计。
