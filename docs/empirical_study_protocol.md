...
### CDC WONDER 分层出生分子：暂缓接入（2026-09-03）

48 个州分批导出需要重复执行浏览器查询；当前仅保存了 `2010-00.tsv`，其余
47 批次为 `pending`。仓库没有可靠的非交互下载器，因此本项暂时冻结，不用部分
结果拼接或插补来冒充完整出生分子。解冻条件是：2010—2017 全部 48 批次通过字段、
州覆盖、抑制值和总量审计，并另行准备 2018—2021 test 出生分子。冻结期间继续使用
已有州级总出生面板进行软件测试、接口验证和方法开发，但不宣称完成年龄×婚姻×孩次
的正式校准或历史回放。
