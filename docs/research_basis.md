...
- 联合国《World Population Prospects 2024》把年龄—性别别生育、死亡和迁移作为人口投影的基本输入，并用概率情景表达未来不确定性：
...
- Human Fertility Database 提供年龄别、胎次别生育率，并区分生育数量（quantum）与生育时间（tempo）；这支持把“想生几个”和“何时生”分开建模：
  https://www.humanfertility.org/Project/Overview
- 人口转变研究强调死亡下降、教育、人力资本和子女质量—数量权衡的动态反馈，而不是单一的“富裕所以少生”规则：
  https://pubmed.ncbi.nlm.nih.gov/11618374/

### 模型映射

- 代码提供年龄率表接口；没有数据时才回退到透明的示意曲线。
- `amenity_supply` 是地区公共服务的综合占位变量，影响迁移效用和生育意愿；它不是现实数据，也不应被解释为单一幸福指数。
- 低宜居度在照护缺口较大时形成软阈值，模拟“住房、托育、学校和就业同时紧张”的叠加，而不是硬性人口上限。
- 下一步应把综合宜居度拆成可观测组件，并用地区—年份数据估计各组件的边际效应与滞后效应。

## 9. 游戏机制的借鉴边界

《文明 VI》官方介绍明确把城市扩张、科技/文化解锁、政府、忠诚度和环境冲击作为长期反馈系统；这些设计适合借鉴“少数可读旋钮、多个相互作用的局部反馈”，不适合直接当作人口学证据：
https://civilization.2k.com/en-GB/civ-vi/

本项目目前只借鉴三点：地区服务供给、人口压力的软惩罚、政策和环境冲击的可追踪历史。游戏中的胜利条件、固定科技树和离散人口单位不进入现实解释层。
